...
export async function loadUsers() {
  // Минимальный, максимально совместимый вызов user.get: без SORT/ORDER/FILTER
  // в запросе — некоторые порталы Битрикс24 отвечают на них 400 Bad Request
  // (а BX24 JS SDK на ошибочный HTTP-статус не вызывает колбэк, и попап «висел»
  // до таймаута). Неактивных отсеиваем и сортируем на стороне клиента.
  const rows = await callListMethod('user.get', {});
  return rows
    .filter((u) => u && u.ID && u.ACTIVE !== false && u.ACTIVE !== 'N')
    .map((u) => ({
      id: String(u.ID),
